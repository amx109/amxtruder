...
use </home/amran/code/extruder/amxtruder/amxtruder.scad>
...
// mirrored mgs
//	mirror([0,0,1]) translate([shafts_distance/2,0,0])mgs();

// fan mount
      rotate([0,180,0]) fan_mount();
// mirrored fan mount
//     mirror([0,0,1]) rotate([0,180,0]) fan_mount();
    ...
    difference(){
		...
		translate([-10,filament_hole_offset,0]){
			rotate(a=0, v=[1,0,0]){
				...
				if (nut_traps) translate([11,0,25]) rotate([0,90,0]) cylinder(r=3.6, h=5, $fn=6); // nut trap
...
module mgs_groovemount(){
    difference(){
        ...
        translate([-10,filament_hole_offset,0]){
            ...
            //hole for the jhead
            translate(v=[33.4-2,0,31.8]) rotate([0,90,0]) 
            cylinder(r=groovemount_radius+0.5,h=groovemount_thickness+2,$fn=64);
            ...
            difference(){    
                union(){
                   // translate([18,5,32]) rotate([90,0,90]) hotend();
					...
					difference()
					{
						translate([23.3,20,0]) cube([37,6,47+20]);
						// cut groovemount
						
						translate([-10,filament_hole_offset,0])
						{
							translate(v=[33.4-2,0,31.8]) rotate([0,90,0]) 
							//cut out for jhead groove
							cylinder(r=groovemount_radius+0.5,
									h=groovemount_thickness+2,$fn=64);
							//cut out in rear wallfor the jhead 
							translate(v=[38,0,31.8]) rotate([0,90,0]) 
								cylinder(r=8.5,h=30.6,$fn=64);
							//cut out for the hot bit of the jhead
							translate(v=[66.5,-1,44]) rotate([0,90,0]) 
								cube([25,15,15]);
						}
					}
                    translate([23.3,-12,0]) cube([37,6,21]);
                    // support for this wall:
                    translate([23.3+6,-9,0]) rotate([0,0,45]) cube([3,6,21]);
                    
                    //ZIP TIE BIT
                    translate([23.3,-10,51]) cube([37,4,15]);
                ...
                // big cuts
                translate([22.3,-38,-10]) cube([39,26,86]);
                translate([22.3,-18,67]) cube([39,50,10]);
                
              

            }
            // THE ZIP TIE BIT - extend one wall to attach wires with zipties       
            difference(){
                translate([23.3,-12,51]) rotate([0,0,0]) cube([37,3,16]);
                translate([40.5,-15,59]) rotate([0,90,0]) ziptie(6,4);        
            }
        }
       // VOID FOR THE ZIP TIE BIT - cutout for extruder wiring     
            translate([50,-16,36+20]) cube([13,16,6]); 
            
            translate([50,-4,39+20]) rotate(a=90,v=[1,0,0]) cylinder(r=3,h=10);                                
    }                
